...
describe('resource articles', () => {
  ...
  test.skip('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.news.articles.list({
  ...
  test.skip('createFeedback: only required params', async () => {
    const responsePromise = client.news.articles.createFeedback({ article: '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test.skip('listSavedArticleFilters: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.news.articles.listSavedArticleFilters({ next_key: 'next_key' }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(BusinessRadar.NotFoundError);
  });
