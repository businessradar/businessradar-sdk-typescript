...
import { AbstractPage, type NextKeyParams, NextKeyResponse } from './core/pagination';
import * as Uploads from './core/uploads';
import * as API from './resources/index';
import { APIPromise } from './core/api-promise';
import { BlankEnum, Companies, CompanyCreateFeedbackParams, CompanyCreateFeedbackResponse, CompanyCreateMissingCompanyInvestigationParams, CompanyCreateMissingCompanyInvestigationResponse, CompanyCreateParams, CompanyListAttributeChangesParams, CompanyListAttributeChangesResponse, CompanyListAttributeChangesResponsesNextKey, CompanyListMissingCompanyInvestigationsParams, CompanyListMissingCompanyInvestigationsResponse, CompanyListMissingCompanyInvestigationsResponsesNextKey, CompanyListParams, CompanyListResponse, CompanyListResponsesNextKey, CompanyRetrieveMissingCompanyInvestigationResponse, CompanyRetrieveResponse, CountryEnum, IndustryCode, Registration, RegistrationRequest } from './resources/companies';
import { Compliance, ComplianceCheckScoreEnum, ComplianceCreateParams, ComplianceCreateResponse, ComplianceListParams, ComplianceListResponse, ComplianceListResponsesNextKey, ComplianceListResultsParams, ComplianceListResultsResponse, ComplianceListResultsResponsesNextKey, ComplianceRetrieveResponse } from './resources/compliance';
import { News } from './resources/news/news';
import { PermissionEnum, Portfolio, PortfolioCreateParams, PortfolioListParams, Portfolios, PortfoliosNextKey } from './resources/portfolios/portfolios';
...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['BUSINESS_RADAR_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['BUSINESS_RADAR_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Business Radar API. 
 */
export class BusinessRadar {
  apiKey: string;
   ...
   * API Client for interfacing with the Business Radar API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['BUSINESSRADAR_API_KEY'] ?? undefined]
   * @param {string} [opts.baseURL=process.env['BUSINESS_RADAR_BASE_URL'] ?? https://api.businessradar.com] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('BUSINESS_RADAR_BASE_URL'),
    apiKey = readEnv('BUSINESSRADAR_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.BusinessRadarError(
        'The BUSINESSRADAR_API_KEY environment variable is missing or empty; either provide it, or instantiate the BusinessRadar client with an apiKey option, like new BusinessRadar({ apiKey: \'My API Key\' }).'
      );
    }
    ...
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `https://api.businessradar.com`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? BusinessRadar.DEFAULT_TIMEOUT /* 1 minute */;
    this.logger = options.logger ?? console;
    const defaultLogLevel = 'warn';
    // Set default logLevel early so that we can log a warning in parseLogLevel.
    this.logLevel = defaultLogLevel;
    this.logLevel = parseLogLevel(options.logLevel, 'ClientOptions.logLevel', this) ?? parseLogLevel(readEnv('BUSINESS_RADAR_LOG'), 'process.env[\'BUSINESS_RADAR_LOG\']', this) ?? defaultLogLevel;
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://api.businessradar.com';
  }
  ...
  requestAPIList<Item = unknown, PageClass extends Pagination.AbstractPage<Item> = Pagination.AbstractPage<Item>>(
    ...
    return new Pagination.PagePromise<PageClass, Item>(this as any as BusinessRadar, request, Page);
  }
  ...
  static BusinessRadar = this;
  static DEFAULT_TIMEOUT = 60000 // 1 minute

  static BusinessRadarError = Errors.BusinessRadarError;
...
export declare namespace BusinessRadar {
      ...
      export import NextKey = Pagination.NextKey;
export {
  type NextKeyParams as NextKeyParams,
  type NextKeyResponse as NextKeyResponse
};
