...
export function codeTool(): McpTool {
  ...
  const handler = async (client: BusinessRadar, args: any): Promise<ToolCallResult> => {
    ...
    const res = await fetch(codeModeEndpoint, {
      ...
      headers: {
        ...
        client_envs: JSON.stringify({
          BUSINESSRADAR_API_KEY: requireValue(
            readEnv('BUSINESSRADAR_API_KEY') ?? client.apiKey,
            'set BUSINESSRADAR_API_KEY environment variable or provide apiKey client option',
          ),
          BUSINESS_RADAR_BASE_URL: readEnv('BUSINESS_RADAR_BASE_URL') ?? client.baseURL ?? undefined,
        }),
      },
      body: JSON.stringify({
        project_name: 'businessradar',
        code,
        intent,
        client_opts: {},
