...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { BUSINESSRADAR_API_KEY: requireValue(readEnv('BUSINESSRADAR_API_KEY') ?? client.apiKey, 'set BUSINESSRADAR_API_KEY environment variable or provide apiKey client option'), BUSINESS_RADAR_BASE_URL: readEnv('BUSINESS_RADAR_BASE_URL') ?? client.baseURL ?? undefined };
...
const localDenoHandler = async (
  ...
  try {
    const sdkPkgName = '@businessradar/businessradar';
